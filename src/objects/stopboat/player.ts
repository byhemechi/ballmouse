...
export default class Player extends Entity {
    ...
    private switchWeapons() {
        ...
        this.currentWeapon =
            (this.weapons.length + this.currentWeapon) % this.weapons.length
        ...
        for (let i = 0; i < this.weapons.length; i++) {
            ...
            if (i == this.currentWeapon) {
                ...
                if (wpn.chargeMax == 0 && wpn.timer > wpn.fireRate) {
                    ...
                    // If this weapon is a charge shot
    ...
    private checkCollision(delta: number) {
        for (let i = 0; i < this.root.enemyBullets.children.length; i++) {
            ...
            if (
                (nextX - this.position.x) ** 2 +
                    (nextY - this.position.y) ** 2 <
                this.size ** 2
            ) {
                this.hurt(eb.damage)
                eb.damage = 0
    ...
    private tryShoot() {
        ...
        if (!blitzing) {
            ...
            if (wpn.chargeMax == 0) {
                ...
                // Do if the weapon IS a charge shot
                if (this.game.keyJustReleased('KeyZ')) {
                    // Calculate shot count
                    wpn.shotCount = Math.round(
                        Math.min(wpn.chargeTimer / wpn.chargeTime, 1) *
                            wpn.chargeMax
                    )
                    // Reset timers
                    wpn.chargeTimer = 0
    ...
    private deathBlitzIncrease(delta: number) {
        ...
        if (this.invincibleTimer > 0) {
            ...
            this.ammo[this.blitzWeapons[this.currentBlitzWeapon].ammoType] > 0
        ) {
            this.children.forEach(i => {
                i.visible = !i.visible
            })
    ...
    private checkBlitz() {
        ...
        if (this.game.keyJustPressed('KeyX') && !this.blitzCooldownTimer) {
            ...
            if (this.root.scoreMultiplier >= 3) {
                ...
                this.ammo[
                    this.blitzWeapons[this.currentBlitzWeapon].ammoType
                ] = this.MAXAMMO[
                    this.blitzWeapons[this.currentBlitzWeapon].ammoType
                ]
    ...
    private hurt(damage: number) {
        if (
            this.ammo[this.blitzWeapons[this.currentBlitzWeapon].ammoType] <=
                0 &&
            this.invincibleTimer <= 0 &&
            this.damage == 0
        ) {
    ...
    private shoot(wpn: Weapon, position: Vector) {
        for (let i = 0.5; i < wpn.shotCount; i++) {
            ...
            if (wpn.spreadIsRandom) {
                angle =
                    this.weaponSpread() * wpn.angularSpread +
                    wpn.tilt * this.keyboardMove()
                variance = this.weaponSpread() * wpn.linearSpread
            } else {
                if (wpn.chargeMax > 1) {
                    variance =
                        ((wpn.shotCount / wpn.chargeMax) *
                            wpn.linearSpread *
                            i) /
                            wpn.shotCount -
                    ...
                    angle =
                        ((wpn.shotCount / wpn.chargeMax) *
                            ...
                            i) /
                            wpn.shotCount -
                        ((wpn.shotCount / wpn.chargeMax) * wpn.angularSpread) /
                            2 +
                        wpn.tilt * this.keyboardMove()
                } else {
                    variance =
                        (wpn.linearSpread * i) / wpn.shotCount -
                        wpn.linearSpread / 2

                    angle =
                        (wpn.angularSpread * i) / wpn.shotCount -
                        wpn.angularSpread / 2 +
                        wpn.tilt * this.keyboardMove()
                }
            }
